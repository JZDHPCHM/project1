 ...
 */
@Service
public class FbJudicialAssistanceServiceImpl extends DaoServiceImpl<FbJudicialAssistanceEntity, String, FbJudicialAssistanceRepository> implements FbJudicialAssistanceService{
    ...
    public Map<String, Object> getJudicialAssistance(String companyId) throws Exception {
        ...
        //获取封装后的url
        Map<String, Object> requestUrlMap = FbHttpUtil.getRequestUrlMap(companyId, dictionaryMap, FbInterfaceEnums.SFXZ.getValue());
        ...
        //递归获取所有数据
        Map<String, Object> result = FbBeanSetValueUtil.getJsonResultMap(FbJudicialAssistanceEntity.class,
                new ArrayList<FbJudicialAssistanceEntity>(), companyId,
                requestUrlMap.get(FbCommonEnums.APPUTIL_MAP_DATA.getValue()).toString(), "",
                EntityMap.judicialAssistanceEntityMap,accessLogService);
        
        if(!FbCommonUtil.getApputilMapFlag(result)) {
            return result;
        }
        List<FbJudicialAssistanceEntity> list = (List<FbJudicialAssistanceEntity>) result.get(FbCommonEnums.APPUTIL_MAP_DATA.getValue());
//        add(list);
        FbBeanSetValueUtil.saveDataToDB(list,FbJudicialAssistanceEntity.class,jdbcTemplate);
        return AppUtil.getMap(true,FbCommonEnums.RESULT_SUCCESS.getValue());
    }

    @Override
    public FbJudicialAssistanceEntity parseJsonObjectToEntity(JSONObject detailObject, String companyId) throws Exception {
        FbJudicialAssistanceEntity entity = FbBeanSetValueUtil.setValue(FbJudicialAssistanceEntity.class, detailObject, EntityMap.judicialAssistanceEntityMap);
