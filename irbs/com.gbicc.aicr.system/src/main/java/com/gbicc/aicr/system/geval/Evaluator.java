...
import com.gbicc.aicr.system.geval.function.math.MathFunctions;
import com.gbicc.aicr.system.geval.function.string.StringFunctions;
import com.gbicc.aicr.system.geval.operator.AdditionOperator;
import com.gbicc.aicr.system.geval.operator.BooleanAndOperator;
import com.gbicc.aicr.system.geval.operator.BooleanNotOperator;
import com.gbicc.aicr.system.geval.operator.BooleanOrOperator;
import com.gbicc.aicr.system.geval.operator.ClosedParenthesesOperator;
import com.gbicc.aicr.system.geval.operator.DivisionOperator;
import com.gbicc.aicr.system.geval.operator.EqualOperator;
import com.gbicc.aicr.system.geval.operator.GreaterThanOperator;
import com.gbicc.aicr.system.geval.operator.GreaterThanOrEqualOperator;
import com.gbicc.aicr.system.geval.operator.LessThanOperator;
import com.gbicc.aicr.system.geval.operator.LessThanOrEqualOperator;
import com.gbicc.aicr.system.geval.operator.ModulusOperator;
import com.gbicc.aicr.system.geval.operator.MultiplicationOperator;
import com.gbicc.aicr.system.geval.operator.NotEqualOperator;
import com.gbicc.aicr.system.geval.operator.OpenParenthesesOperator;
import com.gbicc.aicr.system.geval.operator.Operator;
import com.gbicc.aicr.system.geval.operator.SubtractionOperator;
 ...
 * <ul>
 * <li><i>mathematical</i> Expression involving numbers. Numbers are treated as
 * doubles, so resulting numbers will contain at least one decimal place.</li>
 ...
 * <li><i>functional</i> Custom functions can be created or there are many Math
 * and String functions that JEval supplies with this class.</li>
 * </ul>
 ...
 * <li>> greater than (for numbers and strings)</li> <li>>= greater than or
 * equal (for numbers and strings)</li>
 * <li>&& boolean and</li>
 * <li>|| boolean or</li>
 * <li>! boolean not</li>
 * </ul>
 ...
 * lowest.</li>
 * <li>The expression is evaluated as one or more subexpressions. Subexpressions
 * within open parentheses and closed parentheses are evaluated before other
 * parts of the expression.</li>
 ...
 * <li>Operators with with the same precedence are evaluated from left to right.
 * </li>
 ...
 * <li>Once all variables and functions are resolved, then the parsed expression
 * and subexpressions are evaluated according to operator precedence.</li>
 * </ul>
 * Operator precedence:
 * <ul>
 ...
 * needed.</li>
 * <li>If an expression does not change, it will not be parsed each time the
 * expression is evaluated. Therefore, variables values can change and the
 * expression can be evaluated again without having to re-parse the expression.</li>
...
public class Evaluator {
	 ...
	 * 
	 * @param variables
	 *            The map of variables for this object.
	 */
	...
	/**
	 * Returns the variable resolver. The variable resolver can be used by the
	 * user to resolve their own variables. Variables in the variable resolver
	 * override any variables that are in this classes internal variable map.
	 * 
	...
	public void parse(final String expression) throws EvaluationException {
		...
		try {
			if (parse) {
				...
				// length.
				// 通过遍历字符串表达式(一个一个字节)获取相对应的操作对象和操作符号
	...
	private String getResult(final Stack operatorStack,
		...
		// Check if the final operand is an operand.
			...
			// Get the final result.
			try {
				FunctionResult functionResult = function.execute(this,
						arguments);
			...
			if (openIndex >= 0) {
				...
				if (closedIndex > openIndex) {
					...
					try {
						...
						throw new EvaluationException(
								"Invalid variable name of \"" + variableName
										+ "\".", iae);
					}
