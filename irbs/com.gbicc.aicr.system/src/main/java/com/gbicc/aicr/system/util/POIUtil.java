...
public class POIUtil {
    		...
    		if(cell.getCellTypeEnum() == CellType.STRING){
    			...
    			if(DateUtil.isCellDateFormatted(cell)) {
                    return DateFormatUtils.format(cell.getDateCellValue(),
                            PoiEnum.DEFAULT_DATE_FORMAT_PATTERN.getValue());
    			}else {
	 ...
	 * @return
	 */
	public static boolean isMergedRegion(Sheet sheet, Integer rowNum, Integer cellNum) {
	 ...
	 * @throws Exception
	 */
	public static Object getMergedRegionValue(Sheet sheet, int row, int column) throws Exception {
		int sheetMergeCount = sheet.getNumMergedRegions();
		for (int i = 0; i < sheetMergeCount; i++) {
