...
import com.gbicc.aicr.system.flowable.exception.FlowableRuntimeException;
...
public class FlowableTaskServiceImpl implements FlowableTaskService {
	...
	public void setOtherTaskInfo(List<Map<String, Object>> taskInfoList) throws Exception {
		...
		for(Map<String, Object> map : taskInfoList) {
			...
			if(GeneralUtil.strObjIsEmpty(collUserLoginName)) {
    ...
    @Override
    public List<Map<String, Object>> findAssignInfoByCondition(String bussinessKey, String actId) throws Exception {
        if (StringUtils.isEmpty(bussinessKey) || StringUtils.isEmpty(actId)) {
            return null;
        }
        String sql = "SELECT DISTINCT HA.ASSIGNEE_ LOGINNAME,U.FD_USERNAME USERNAME,ORG.FD_NAME ORGNAME FROM ACT_HI_PROCINST HP INNER JOIN ACT_HI_ACTINST HA ON HP.PROC_INST_ID_=HA.PROC_INST_ID_\r\n"
                ...
                + "INNER JOIN FR_AA_ORG ORG ON UO.FD_ORG_ID=ORG.FD_ID WHERE HP.BUSINESS_KEY_='" + bussinessKey
                + "' AND HA.ACT_ID_='" + actId + "'";
    ...
    public Map<String, Object> findProcessComment(String id, Integer size, Long page) throws Exception {
        if (StringUtils.isEmpty(id)) {
            throw new FlowableRuntimeException("评估项目ID为空!");
        }
        HistoricProcessInstance processInstance = historyService.createHistoricProcessInstanceQuery()
                .processInstanceBusinessKey(id).singleResult();
        List<TaskInfo> comments = myFinishedTaskDetailByProIns(processInstance.getId());
