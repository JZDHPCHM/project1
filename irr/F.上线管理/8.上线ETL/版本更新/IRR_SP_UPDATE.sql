DROP PROCEDURE SP_MDS_PLATINUM_HRM
/
DROP PROCEDURE SP_MDS_DTL_EMPLOYEE_LOSS
/
...
--最近4个季度省级分公司及以下分支机构销售、承保、保全部门离职员工人数
 SELECT 
            C.CODE org,
           DECODE(B.CODE,''GD'',''GP'',''BD'',''BK'',''RP_BRD'',''RP'',''ID'',''FC'',''IDSD'',''FC'',''IDSS'',''FC'',''IDSUD'',''FC'',''IDT'',''FC'',''SFD'',''FC'',''SUD'',''FC'',B.CODE) CHANNEL ,
           SUM(A3) cnt1,
           0 cnt2,
           0 cnt3
 FROM E_BDS_HRM_EMPLOYEESTATE A
  JOIN E_BDS_HRM_ORGANIZATION B
    ON A.ORGCODE4 = B.CODE
     AND B.CODE IN (''RP_BRD'',''AD'',''BD'',''GD'',''CS'',''ID'',''IDSD'',''IDSS'',''IDSUD'',''IDT'',''SFD'',''SUD'') --RP_BRD 续期 AD 多元 BD 银保 GD 团险 CS 客服 ID IDSD IDSS IDSUD IDT SFD SUD 个险
 ...
 GROUP BY C.CODE,DECODE(B.CODE,''GD'',''GP'',''BD'',''BK'',''RP_BRD'',''RP'',''ID'',''FC'',''IDSD'',''FC'',''IDSS'',''FC'',''IDSUD'',''FC'',''IDT'',''FC'',''SFD'',''FC'',''SUD'',''FC'',B.CODE)
 UNION ALL
...
--最近4个季度省级分公司及以下分支机构销售、承保、保全部门增加员工人数
 ...
 AND  a.orgcode2 <> ''HO''
 AND SUBSTR(A.DATA_DATE,1,6) BETWEEN SUBSTR('''||V_EVAL_ENDTIME||''',1,6) AND '''||V_EVAL_DATE||'''
 GROUP BY C.CODE, DECODE(B.CODE,''GD'',''GP'',''BD'',''BK'',''RP_BRD'',''RP'',''ID'',''FC'',''IDSD'',''FC'',''IDSS'',''FC'',''IDSUD'',''FC'',''IDT'',''FC'',''SFD'',''FC'',''SUD'',''FC'',B.CODE)) t 
...
BEGIN
  ...
  V_SQL   := 'INSERT INTO E_MDS_IRR_INDEX_VALUE
    ...
    SELECT ''OR02002'',
           ''86'',
          DECODE(T.CODE,''GD'',''GP'',''BD'',''BK'',''RP_BRD'',''RP'',''ID'',''FC'',''IDSD'',''FC'',''IDSS'',''FC'',''IDSUD'',''FC'',''IDT'',''FC'',''SFD'',''FC'',''SUD'',''FC'',T.CODE) CHANNEL,
           SUM(T.A3),
...
and B.CODE IN (''RP_BRD'',''AD'',''BD'',''GD'',''CS'',''ID'',''IDSD'',''IDSS'',''IDSUD'',''IDT'',''SFD'',''SUD'')) T
GROUP BY T.CODE';
  EXECUTE IMMEDIATE V_SQL;
    COMMIT;
  V_RECORD_NUM := SQL%ROWCOUNT;
...
AND B.CODE IN (''RP_BRD'',''AD'',''BD'',''GD'',''CS'',''ID'',''IDSD'',''IDSS'',''IDSUD'',''IDT'',''SFD'',''SUD'')
AND b.typeid=''4''
LEFT JOIN E_BDS_HRM_ORG C
ON A.ORGCODE2 = C.ORGCODE
