...
public class IrrSplitIndexServiceImpl extends DaoServiceImpl<IrrSplitIndexEntity, String, IrrSplitIndexRepository> 
	...
	public List<IrrSplitIndexEntity> findSplitIndexByCondition(String isUse, String collWay, String indexStatus,
		...
		StringBuffer sb = new StringBuffer("SELECT SI.* FROM T_IRR_SPLIT_INDEX SI "
				+ "INNER JOIN T_IRR_INDEX_INFO II ON SI.SOURCE_INDEX_ID=II.ID "
				+ "INNER JOIN T_IRR_PROJ_TYPE PT ON II.PROJ_TYPE_ID=PT.ID WHERE PT.IS_USE='"+IrrEnum.YESNO_Y.getValue()+"' ");
		if(StringUtils.isNotBlank(isUse)) {
			sb.append("AND SI.IS_USE='"+isUse+"' ");
		}
		...
		if(StringUtils.isNotBlank(orgId)) {
			...
			sb.append("AND SI.ORG_ID IS NULL ");
		}
        sb.append("ORDER BY II.PROJ_TYPE_ID,II.INDEX_LINE");
		return jdbcTemplate.query(sb.toString(), new BeanPropertyRowMapper<IrrSplitIndexEntity>(IrrSplitIndexEntity.class));
	}
	...
	@Override
	public Map<String, Map<String,Object>> findSplitSourceIndexInfo() throws Exception {
		String sql = "SELECT DISTINCT II.PROJ_TYPE_NAME,II.INDEX_LINE,SI.SPLIT_CODE FROM T_IRR_INDEX_INFO II "
				+ "INNER JOIN T_IRR_SPLIT_INDEX SI ON II.ID=SI.SOURCE_INDEX_ID "
				+ "INNER JOIN T_IRR_PROJ_TYPE PT ON II.PROJ_TYPE_ID=PT.ID WHERE PT.IS_USE='"+IrrEnum.YESNO_Y.getValue()+"'";
		...
		for(Map<String, Object> temp : list) {
			if(MapUtils.isEmpty(temp) || IrrUtil.strObjectIsEmpty(temp.get("SPLIT_CODE"))) {
				continue;
			}
			map.put(temp.get("SPLIT_CODE").toString(), temp);
		}
		return map;
	}

	@Override
	public List<IrrProjTypeEntity> findProjBySplitIndexCondition(String splitLevel, Boolean channelFlagIsNull)
			throws Exception {
		String sql = "SELECT DISTINCT PROJ.* FROM T_IRR_PROJ_TYPE PROJ INNER JOIN T_IRR_SPLIT_INDEX SI ON PROJ.ID=SI.SOURCE_PROJ_ID WHERE 1=1 ";
		if(StringUtils.isNotBlank(splitLevel)) {
			sql += "AND SI.SPLIT_LEVEL='"+splitLevel+"' ";
		}
		if(channelFlagIsNull != null) {
			if(channelFlagIsNull) {
				sql += "AND SI.CHANNEL_FLAG IS NULL ";
			}else {
				sql += "AND SI.CHANNEL_FLAG IS NOT NULL ";
			}
		}
    ...
    @Override
    public Map<String, Object> findSplitIndex(String data, Long page, Integer size) throws Exception {
        StringBuffer sb = new StringBuffer(
                "SELECT ISI.* FROM T_IRR_SPLIT_INDEX ISI INNER JOIN T_IRR_INDEX_INFO II ON ISI.SOURCE_INDEX_ID=II.ID WHERE 1=1 ");
        if (StringUtils.isNotBlank(data)) {
            JSONObject json = JSONObject.fromObject(data);
            if (json.has("projTypeId") && StringUtils.isNotBlank(json.getString("projTypeId"))) {
                sb.append("AND ISI.SOURCE_PROJ_ID='" + json.getString("projTypeId") + "' ");
            }
            if (json.has("splitCode") && StringUtils.isNotBlank(json.getString("splitCode"))) {
                sb.append("AND ISI.SPLIT_CODE LIKE '%" + json.getString("splitCode") + "%' ");
            }
            if (json.has("splitName") && StringUtils.isNotBlank(json.getString("splitName"))) {
                sb.append("AND ISI.SPLIT_NAME LIKE '%" + json.getString("splitName") + "%' ");
            }
            if (json.has("collWay") && StringUtils.isNotBlank(json.getString("collWay"))) {
                sb.append("AND ISI.COLL_WAY='" + json.getString("collWay") + "' ");
            }
            if (json.has("splitLevel") && StringUtils.isNotBlank(json.getString("splitLevel"))) {
                sb.append("AND ISI.SPLIT_LEVEL='" + json.getString("splitLevel") + "' ");
            }
            if (json.has("isUse") && StringUtils.isNotBlank(json.getString("isUse"))) {
                sb.append("AND ISI.IS_USE='" + json.getString("isUse") + "' ");
            }
            sb.append("ORDER BY II.INDEX_LINE ");
        } else {
            sb.append("ORDER BY ISI.SPLIT_CODE ");
        }
        List<IrrSplitIndexEntity> list = jdbcTemplate.query(sb.toString(),
                new BeanPropertyRowMapper<IrrSplitIndexEntity>(IrrSplitIndexEntity.class));
        ResponsePage<IrrSplitIndexEntity> response = new ResponsePage<IrrSplitIndexEntity>();
