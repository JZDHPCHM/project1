...
public class IrrIndexInfoServiceImpl extends DaoServiceImpl<IrrIndexInfoEntity, String, IrrIndexInfoRepository> 
	...
	public List<IndexValueDTO> sumHeadChannelIndex(String taskId) throws Exception {
		String sql = "SELECT II.ID INDEX_ID,II.INDEX_CODE CODE,II.INDEX_NAME NAME,SUM(NVL(UR.SPLIT_RESULT_Q2,0)) VALUE\r\n" + 
				...
				"AND II.IS_XBRL='"+IrrEnum.YESNO_Y.getValue()+"' AND SI.IS_USE='"+IrrEnum.YESNO_Y.getValue()+"' AND SI.SPLIT_LEVEL='"+IrrEnum.SPLIT_LEVEL_CHANNEL.getValue()+"' GROUP BY II.ID,II.INDEX_CODE,II.INDEX_NAME";
		return jdbcTemplate.query(sql, new BeanPropertyRowMapper<IndexValueDTO>(IndexValueDTO.class));
	}
	
	@Override
	public Map<String, Object> importFileInfoBytempId(HSSFWorkbook wb ,String templateId ,String projTypeId,String taskBatch) {
		
		Map<String, Object> map = new HashMap<String, Object>();
		
		try {
			if (templateId == "OR11" || templateId.equals("OR11")) {
				map = importCorpgoveResult(wb ,projTypeId,taskBatch);
			} else if (templateId == "OR16" || templateId.equals("OR16")) {
				map = importInfosysResult(wb ,projTypeId,taskBatch);
			} else if (templateId == "OR17" || templateId.equals("OR17")) {
				map = importCasemanaResult(wb ,projTypeId,taskBatch);
			} else if (templateId == "LR01" || templateId.equals("LR01")) {
				map = importLiquriskResult(wb ,projTypeId,taskBatch);
			} else if (templateId == "SR01" || templateId.equals("SR01")) {
				map = importStrariskResult(wb ,projTypeId,taskBatch);
			}
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 * @return
	 */
	private Map<String, Object> importCorpgoveResult(HSSFWorkbook wb ,String projTypeId,String taskBatch) {
			...
			for (int numSheet = 0; numSheet < wb.getNumberOfSheets(); numSheet++) {
				...
				for (int rowNum = 2; rowNum <= totalRows; rowNum++) {
					...
					if (hssfRow != null) {
						...
						// 初始化对象数据
						cellNum=cellNum+2;
						corpgove.setIndexName(StringUtils.isBlank(StringgetCellString(hssfRow.getCell(cellNum)))?null:StringgetCellString(hssfRow.getCell(cellNum)).trim());
						cellNum++;
						corpgove.setEvalPoint(StringUtils.isBlank(StringgetCellString(hssfRow.getCell(cellNum)))?null:StringgetCellString(hssfRow.getCell(cellNum)).trim());
						cellNum++;
						corpgove.setIndexCode(StringgetCellString(hssfRow.getCell(cellNum)).trim());
						cellNum++;
						corpgove.setEvalCrit(StringgetCellString(hssfRow.getCell(cellNum)).trim());
						cellNum+=2;
	 ...
	 * @return
	 */
	private Map<String, Object> importInfosysResult(HSSFWorkbook wb ,String projTypeId,String taskBatch) {
			...
			for (int numSheet = 0; numSheet < a; numSheet++) {
				...
				for (int rowNum = 1; rowNum <= totalRows; rowNum++) {
					...
					if (hssfRow != null) {
						...
						// 初始化对象数据
						obj = POIUtil.getCellValue(hssfRow.getCell(cellNum));
                        infosys.setIndexCode(obj == null ? null : obj.toString());
						cellNum++;
						obj = POIUtil.getCellValue(hssfRow.getCell(cellNum));
                        infosys.setIndexName(obj == null ? null : obj.toString());
						cellNum++;
						obj = POIUtil.getCellValue(hssfRow.getCell(cellNum));
						infosys.setEvalCrit(obj == null ? null : obj.toString());
                        cellNum++;
						obj = POIUtil.getCellValue(hssfRow.getCell(cellNum));
						infosys.setIndexValue(obj == null ? null : obj.toString());
						cellNum++;
						obj = POIUtil.getCellValue(hssfRow.getCell(cellNum));
						infosys.setIndexScore(obj == null ? null : obj.toString());
						cellNum++;
						obj = POIUtil.getCellValue(hssfRow.getCell(cellNum));
						infosys.setIndexCentScore(obj == null ? null : obj.toString());
	 ...
	 * @return
	 */
	private Map<String, Object> importCasemanaResult(HSSFWorkbook wb ,String projTypeId,String taskBatch) {
			...
			for (int numSheet = 0; numSheet < wb.getNumberOfSheets(); numSheet++) {
				...
				for (int rowNum = 2; rowNum <= totalRows; rowNum++) {
					...
					if (hssfRow != null) {
						...
						cellNum=cellNum+4;//原3，加了一列，后移一个单位
	 ...
	 * @return
	 */
	private Map<String, Object> importLiquriskResult(HSSFWorkbook wb ,String projTypeId,String taskBatch) {
	 ...
	 * @return
	 */
	private Map<String, Object> importStrariskResult(HSSFWorkbook wb ,String projTypeId,String taskBatch) {
	...
	public List<IrrIndexInfoEntity> findIndexInfoByCondtion(String indexStatus, String indexLevel, String isApply,
		...
		StringBuffer sb = new StringBuffer("SELECT II.* FROM T_IRR_INDEX_INFO II "
				+ "LEFT JOIN T_IRR_SPLIT_INDEX SI ON II.ID=SI.SOURCE_INDEX_ID "
				+ "LEFT JOIN T_IRR_PROJ_TYPE PT ON II.PROJ_TYPE_ID=PT.ID "
				+ "WHERE PT.IS_USE='"+IrrEnum.YESNO_Y.getValue()+"' ");
		...
		if(StringUtils.isNotBlank(isApply)) {
			sb.append("AND II.IS_APPLICABIE='"+isApply+"' ");
		}
		if(StringUtils.isNotBlank(isScore)) {
			sb.append("AND II.IS_SCORE_INDEX='"+isScore+"' ");
		}
		if(formulaIsNull != null) {
			if(formulaIsNull) {
				sb.append("AND II.INDEX_EVAL_FORM IS NULL ");
			}else {
				sb.append("AND II.INDEX_EVAL_FORM IS NOT NULL ");
			}
		}
		if(StringUtils.isNotBlank(isXbrl)) {
			sb.append("AND II.IS_XBRL='"+isXbrl+"' ");
		}
		if(isChannel != null) {
			if(isChannel) {
				sb.append("AND SI.SPLIT_LEVEL='"+IrrEnum.SPLIT_LEVEL_CHANNEL.getValue()+"' ");
			}else {
				sb.append("AND SI.SPLIT_LEVEL<>'"+IrrEnum.SPLIT_LEVEL_CHANNEL.getValue()+"' ");
			}
		}
		return jdbcTemplate.query(sb.toString(), new BeanPropertyRowMapper<IrrIndexInfoEntity>(IrrIndexInfoEntity.class));
	}

	@Override
	public List<IrrIndexInfoEntity> findIndexInfoByCondition(String projId, String indexStatus, String isApply,
			String isScore) throws Exception {
		StringBuffer sb = new StringBuffer("SELECT * FROM T_IRR_INDEX_INFO II WHERE 1=1 ");
		if(StringUtils.isNotBlank(projId)) {
			sb.append("AND II.PROJ_TYPE_ID='"+projId+"' ");
		}
		if(StringUtils.isNotBlank(indexStatus)) {
			sb.append("AND II.INDEX_STATUS='"+indexStatus+"' ");
		}
    ...
    public List<IrrIndexInfoEntity> findIndexInfoHaveFactor(String projId, String indexStatus, String isApply,
        ...
        StringBuffer sb = new StringBuffer(
                "SELECT DISTINCT II.* FROM T_IRR_INDEX_INFO II INNER JOIN T_IRR_INDEX_RELATION IR ON II.ID=IR.INDEX_ID WHERE 1=1 ");
        if (StringUtils.isNotBlank(projId)) {
            sb.append("AND II.PROJ_TYPE_ID='" + projId + "' ");
        }
        if (StringUtils.isNotBlank(indexStatus)) {
        ...
        return jdbcTemplate.query(sb.toString(),
                new BeanPropertyRowMapper<IrrIndexInfoEntity>(IrrIndexInfoEntity.class));
    }
